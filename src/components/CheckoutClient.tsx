...
export default function CheckoutClient() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    else { setError(data.error ?? 'Unable to begin checkout.'); setSubmitting(false); }
  }
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid gap-10 lg:grid-cols-[1.25fr_.75fr]">
          <form onSubmit={submit} className="border border-white/10 bg-[#0e0e0e] p-6 sm:p-9">
            ...
            <p className="mt-4 text-center text-white/30 text-xs">Payments are processed securely by Stripe.</p>
          </form>
          <aside className="h-fit border border-white/10 bg-[#0e0e0e] p-6">
            ...
            <div className="space-y-3 pt-5 text-sm"><div className="flex justify-between text-white/50"><span>Subtotal</span><span>Rs. {subtotal.toLocaleString()}</span></div>{discount > 0 && <div className="flex justify-between text-[#D4AF37]"><span>BOGO saving</span><span>- Rs. {discount.toLocaleString()}</span></div>}<div className="flex justify-between border-t border-white/10 pt-4 text-lg text-white"><span>Total</span><span>Rs. {total.toLocaleString()}</span></div></div>
          </aside>
        </div>
      </div>
    </main>
