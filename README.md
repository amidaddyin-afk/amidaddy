...
7. Set `CRON_SECRET` for `/api/maintenance`, configure both Turnstile keys, and choose `ADMIN_SESSION_MAX_AGE_HOURS` (12 by default). Production authentication fails closed when the Turnstile secret is absent. `vercel.ts` schedules `/api/maintenance` once daily at 03:00 UTC on Vercel Cron — the Hobby plan only allows daily schedules; upgrade to Pro for a tighter interval (requires the project to be deployed on Vercel with `CRON_SECRET` set — Vercel Cron sends it automatically as the `Authorization: Bearer` header).
